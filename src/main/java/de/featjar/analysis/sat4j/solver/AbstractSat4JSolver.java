...
//		Logger.logError(formula.getVariableMap().getVariableCount() + ": "
//				+ formula.getConstraints().stream().filter(Objects::nonNull).map(Object::toString).reduce((a, b) -> a + ", " + b).orElse(""));
		if (contradiction) {
			lastModel = null;
		...
		try {
			if (solver.isSatisfiable(new VecInt(assumptionArray), globalTimeout)) {
				...
				addSolution();
//				Logger.logError("\t"+Arrays.toString(lastModel));
				return SatResult.TRUE;
			} else {
				lastModel = null;
//				Logger.logError("\tNo Solution :(");
				return SatResult.FALSE;
			}
